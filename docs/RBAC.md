...
**版本**: v3.6.5
**更新日期**: 2026-07-08  
**变更说明**: 新增通知模板/规则/监控文档；权限码修正 notification:manage → notification:write；新增 backup:write 权限；完善通知和备份模块权限矩阵；补充审计日志覆盖
...
| `settings:write` | 修改系统配置 | 修改系统配置（含认证提供者管理、邮件配置管理） |
...
| 权限码 | 名称 | 描述 |
|--------|------|------|
| `notification:read` | 查看通知 | 查看通知渠道、模板、规则、日志、统计 |
| `notification:write` | 管理通知 | 创建/编辑/删除通知渠道、模板、规则，测试渠道，手动重试 |

#### 备份管理 (backup)

| 权限码 | 名称 | 描述 |
|--------|------|------|
| `backup:write` | 管理备份 | 配置备份、执行备份/恢复、下载/删除备份文件 |
