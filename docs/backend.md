...
> **v3.6.13 新增：** 解决前端基于当前页数据计算统计不准确的问题，改为服务端全局统计。
...
> v3.10.0 变更：
> - 白名单备份受 `backup_whitelist` 选项控制
> - 配置文件备份新增 `nginx.conf`、`alembic.ini`
> - 系统配置 DB 恢复使用 `begin_nested()` 逐表事务保护
> - 新增远程备份过期清理（SFTP/FTP）
> - 新增白名单恢复、日志恢复方法
> - 手动备份端点加载用户保存的配置
> - PostgreSQL 未配置时抛出异常而非静默成功
> 
> 告警阈值配置化：
> - 新增 `ALERT` 配置分类
> - 合规率告警阈值、危险比例、封锁数量阈值、离线检测倍数从硬编码改为系统配置项
> - 修复合规率告警量纲 bug（0-100 vs 0-1）
